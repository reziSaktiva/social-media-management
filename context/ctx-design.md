...
4. **shadcn/ui adalah fondasi komponen permanen** (ADR-097, membalik ADR-041). Migrasi dari Astryx berjalan incremental per route-segment (lihat `tasks/v07-astryx-shadcn-migration.md`). Tailwind dipakai langsung sebagai styling komponen shadcn; wrapper di `components/ui/` dibuat selektif.
