...
**Terkait KI-049** (Security/Bug, Open, 2026-09-07): halaman `/invite/[token]` (`AcceptInviteForm.tsx`) mengunci field email jadi read-only ke email undangan, tapi tidak memverifikasi kepemilikan inbox — kalau email A belum punya akun dan link terbuka email B, B bisa membajak identitas A saat sign-up. Akar masalah KI-001 (`requireEmailVerification: false`). Belum ada mitigasi.
