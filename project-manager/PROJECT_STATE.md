...
* **T-046 Done (3/3 subtask) — Account Overview (2026-09-18)** — bar performa jumlah post + total reach per akun/platform di `/analyze` (reuse shadcn `Progress`). Method baru `PublishingService.getAccountOverview(workspaceId, period, userId)` ditaruh di domain `publishing` (bukan `analytics`) untuk reuse `getPostPerformance` (T-043) dan menghindari circular dependency — port baru `ConnectedAccountsPort` (arah `publishing→workspace`, sudah legal di `application-layer.md`). UI direflow (2 sesi Mark UI Engineer, `DesignSync get_file` ulang) supaya Account Overview + Post Performance berbagi satu `Card`/`CardContent`, sesuai struktur asli `analyze-dashboard.html` (`.dash-cols` grid `1.6fr 1fr`) — kolom kanan grid (Engagement Summary, T-044) sengaja dirender full-width sementara sampai T-044 dikerjakan, dikonfirmasi King Rezi via `AskUserQuestion`. Kriteria "belum ada data": akun tanpa post → `totalPosts: 0, totalReach: null`; akun dengan post tapi metrik belum ter-ingest → `totalReach: null` juga. Rangkaian: Prabowo Feature Engineer (T-046.1) → Mark UI Engineer (T-046.2–T-046.3) → Ridwan Architecture Reviewer (0 temuan) → Najwa QA Engineer (0 temuan blocking, gate T-103.3 match; 1 catatan non-blocking sort `localeCompare()` kolom Akun). Verifikasi: `typecheck`/`lint` bersih, `vitest` **415 pass/5 skip** (42 file, naik dari 411). Detail: `tasks/v03-analytics-mvp.md` § T-046.
---
