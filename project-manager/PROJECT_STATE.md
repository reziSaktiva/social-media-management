...
* **T-103.1–T-103.3 Done — Kunci pola shadcn + gate proses dua arah (2026-09-10)** — **T-103.1:** audit `components/*.html`/`templates/*.html` Claude Design, 3 file (`publish-drafts.html`, `settings-workspaces.html`, `settings-connected-accounts.html`) yang mockup-nya masih pola lama disamakan penuh dengan kode nyata (`Table` tanpa `TableHeader`/`Card`), termasuk posisi tombol "+ New Post" yang sempat salah (sudah dipindah sejajar judul, mengikuti `PublishPageHeader.tsx`). `readme.md` diupdate ke penanda "SYNCED". PR [#116](https://github.com/reziSaktiva/social-media-management/pull/116) ke `staging`. **T-103.2:** gate ditambahkan ke `AGENTS.md` rule 17 — kalau pola shadcn masih ambigu (belum ada penanda SYNCED/LOCKED), AI wajib berhenti & `AskUserQuestion` ke King Rezi sebelum implementasi, bukan menebak. **T-103.3:** gate kedua ditambahkan setelah implementasi — `mark-ui-engineer.md` & `najwa-qa-engineer.md` wajib `DesignSync get_file` + bandingkan struktur kode vs Claude Design sebelum task UI ditandai selesai; Najwa diberi tool `DesignSync` baru supaya bisa eksekusi sendiri. Kedua gate melengkapi (bukan menggantikan) satu sama lain — dibahas eksplisit dengan King Rezi supaya gate "sebelum" tidak dihapus demi gate "sesudah" saja (persis mekanisme yang gagal di KI-054/055). Izin eksplisit King Rezi untuk edit 3 file read-only. T-103.4 masih `⏳` (opsional). Detail: `tasks/v07-astryx-shadcn-migration.md` § T-103.
