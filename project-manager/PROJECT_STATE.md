...
* **T-007.1/.5/.6 selesai (2026-08-14)** — Members management: invite member jalur **Copy Link** (`WorkspaceService.inviteMember`, ADR-080), dialog konfirmasi Remove Member + Update Member Role (ADR-049 Tier 2), dan UI dialog invite 2 metode (Copy Link aktif, Kirim via Email disabled — email-bound wajib). Lolos review Ridwan (bersih, tanpa temuan) dan QA Najwa (126 test passed + 3 skip pre-existing; golden path Invite Member via Copy Link verified langsung di browser). Heading duplikat "Members" yang sempat muncul saat implementasi sudah diperbaiki lewat slot `headerAction` di `MembersTable`. Remove/Update Role tidak bisa diuji end-to-end penuh karena dev DB baru punya 1 member + env `JOB_SECRET` belum diisi (known gap, KI-015). T-007 tetap 🟡 In Progress — sisa T-007.7 (jalur Kirim via Email) blocked T-005. Detail: `tasks/v01-foundation.md` § T-007, `COMPLETE_TASK.md`.
