...
* **T-103.1 Done + follow-up koreksi King Rezi — Kunci & sinkronkan pola shadcn di Claude Design (2026-09-10)** — audit `components/*.html`/`templates/*.html` project Claude Design lewat `DesignSync`, ditemukan 3 file (`publish-drafts.html`, `settings-workspaces.html`, `settings-connected-accounts.html`) yang mockup-nya masih pola lama (`.queue-list`, `.ws-pick-item`, `.settings-row`) padahal kode nyata `apps/web` sudah dikunci ke pola `Table` final via KI-055. **Follow-up (dikoreksi King Rezi):** posisi tombol "+ New Post" di `publish-drafts.html` dipindah sejajar judul (sebelumnya salah, baris terpisah di bawah tabbar) mengikuti `PublishPageHeader.tsx`; markup ketiga file di atas diubah langsung (bukan cuma komentar) supaya identik strukturnya dengan kode nyata (`WorkspacesSettingsView.tsx`/`ConnectedAccountsList.tsx`/`DraftsList.tsx`). Tabel Components di `readme.md` diupdate ke penanda "SYNCED". Sisa ~28 file dicek sepintas, tidak ditemukan ambiguitas setara — audit mendalam jadi follow-up opsional di T-103.4. T-103.2–T-103.4 masih `⏳`. Detail: `tasks/v07-astryx-shadcn-migration.md` § T-103.
