...
* **KI-059 Resolved — Verifikasi manual browser end-to-end T-024 (Media upload), 2026-09-15** — sesi verifikasi murni (tidak ada perubahan kode), dijalankan di worktree `staging-checkout-tasks-976e14`. Root cause blocker `DATABASE_URL` sebelumnya ternyata bukan env var hilang, tapi dev server sempat start dari `cwd` repo `main` yang salah — setelah dijalankan manual dengan `cwd` worktree yang benar, env terbaca normal. Seluruh 6 kriteria verifikasi **PASS**: upload media (signed URL Supabase Storage asli + `MediaItem` DB baru), Save as Draft, reopen edit, preview restore (thumbnail ter-restore benar, `naturalWidth` cocok), validasi batas count (10 media untuk format `Post` per ADR-107, file ke-11 ditolak), dan hapus permanen (diverifikasi langsung di level DB via `withCurrentUser`/RLS — record benar-benar hilang dari `media_items`, bukan cuma UI). T-024 (5/5 subtask) sekarang **teruji penuh end-to-end**. **KI-060 baru ditemukan** (di luar scope T-024, bukan regresi): Account Selector tidak ter-restore saat reopen edit draft — `getDraftAction` di `Modal.tsx` hanya men-set caption/status/media, tidak `selectedAccountIds`/`formatByAccount`. Dicatat, belum diperbaiki. Detail: `COMPLETE_TASK.md` (2026-09-15).
* **T-024 Done (5/5 subtask) — Media upload di Draft Editor, seluruh rangkaian tuntas, ADR-107 (2026-09-14)** — subtask terakhir **T-024.5** (Delete Media + dialog konfirmasi Tier 2, ADR-049): King Rezi mengonfirmasi via `AskUserQuestion` bahwa tombol "hapus dari post ini" T-024.4 (unlink-only) diganti total jadi aksi destruktif (dialog konfirmasi → hapus file Storage + record `MediaItem` DB permanen). `DeleteMediaUseCase` baru (hapus record DB dulu, baru best-effort hapus file Storage), Server Action `deleteMediaAction`, UI `Modal.tsx` reuse `ConfirmActionDialog`/`useConfirmAction` (pola sama "Hapus Draft"). Lolos review Ridwan (0 temuan). Sebelumnya, **T-024.4** (kontrol lampiran + preview, carousel multi-format): King Rezi mengonfirmasi via `AskUserQuestion` Draft Editor mendukung **multi-media (carousel)**, batas jumlah TERGANTUNG `ContentFormat` (`Post` maks 10, `Reel`/`Story`/`Pin` maks 1; batas efektif = **MINIMUM** dari batas semua format kalau draft menargetkan beberapa akun berformat berbeda sekaligus) — mengamandemen ADR-039 → **ADR-107**. "Pilih dari Media Library" tetap ditunda (link disabled tooltip "Coming soon"). Implementasi Prabowo Feature Engineer (full-stack Server Action → service → repository → UI): `content-format-matrix.ts` (`MAX_MEDIA_COUNT_BY_FORMAT`, `assertMediaCountWithinLimit`), `resolve-draft-media-ids.ts` baru (validasi ownership anti-IDOR), `PublishingService`/`IPublishingRepository` mengaktifkan kolom `media_ids` yang sebelumnya dead dengan partial-update semantics (`undefined` = kolom tidak disentuh, `[]` = dikosongkan), `MediaService` method baru `findByIds`/`listByIds`, Server Action baru `uploadMediaAction` (pertama kalinya domain `media` T-024.1–.3 disambungkan ke UI), UI `Modal.tsx` custom dropzone drag-drop + grid preview multi-media + hapus-per-item. Review Ridwan Architecture Reviewer 2 putaran: putaran 1 — **1 temuan MEDIUM** (partial-update semantics `mediaIds` tidak pernah benar-benar tereksekusi karena `undefined` selalu di-collapse jadi `[]` di titik resolusi awal), diperbaiki (bedakan `undefined` vs `[]` dari awal sampai `PublishingService`, 12 unit test baru); putaran 2 — **0 temuan**. Verifikasi akhir keseluruhan task: `typecheck` 0 error, `lint` 0 error/warning, `vitest` **377 pass/5 skip** (naik dari 362). **KI-059 Resolved (2026-09-15)** — lihat bullet di atas untuk detail verifikasi manual browser end-to-end. Detail: `tasks/v02-publishing-mvp.md` § T-024, `decisions/ADR-107-batas-maksimum-jumlah-media-per-content-format.md`.
* **T-024.1/.2/.3 Done (3/5 subtask) — Domain `media` skeleton + upload Supabase Storage + Fake OutstandAdapter working copy, ADR-106 (2026-09-14)** — T-024 sebelumnya depends penuh T-025.5 (Real OutstandAdapter Media API, terhenti kredensial), diputuskan tetap bisa jalan sekarang mengikuti rule 19 AGENTS.md (pola ADR-059/ADR-105): subtask yang tidak butuh kredensial asli (T-024.1/.2/.4/.5) jalan duluan, T-024.3 dibangun lewat `FakeOutstandAdapter`. Dicek ke Claude Design (rule 17): rancangan Draft Editor sudah ada tapi dropzone media belum dikunci ke primitive shadcn konkret — King Rezi dikonfirmasi via `AskUserQuestion`: dropzone dibangun **custom** (bukan native file input), dan scope T-024 dipersempit (**"Pilih dari Media Library" ditunda**, hanya upload file baru + preview + delete). **T-024.1** (Elon Backend Engineer): skeleton domain `media` — `MediaService`/`IMediaRepository` (murni orchestrate, tanpa Storage/OutstandAdapter), implementasi Prisma (`apps/web/src/lib/repositories/media/`), `MediaType`/`asMediaId` baru di `packages/shared`, 6 unit test. Lolos review arsitektur Ridwan (0 temuan) dan verifikasi independen (`typecheck` 0 error, 338 test pass/5 skip). **T-024.2** (Elon Backend Engineer): upload ke Supabase Storage — `IMediaStorageAdapter`, `UploadMediaUseCase` (validasi mime/ukuran sebelum upload, cleanup best-effort kalau create DB gagal), `SupabaseMediaStorageAdapter` (bucket `media` **Private**, signed URL, path `{workspaceId}/{year}/{month}/{uuid}.{ext}`), migration bucket baru — **sudah dibuat tapi belum dijalankan** (`bun run db:deploy` pending King Rezi). Lolos review Ridwan (0 temuan; 1 catatan non-blocking soal `file_size_limit` belum diisi). King Rezi dikonfirmasi via `AskUserQuestion`: **batas ukuran file media MVP = 50 MB** — diterapkan ke migration/validasi/test (bukan ADR, murni parameter konfigurasi seperti batas 2MB avatar). Verifikasi: `typecheck` 0 error, 344 test pass/5 skip. **T-024.3** (Elon Backend Engineer): kontrak `uploadMediaWorkingCopy` + `FakeOutstandAdapter` (instant always-success, pola ADR-059, `outstandMediaId` acak per panggilan, `expiresAt` mock +24 jam), 2 unit test baru. **ADR-106 baru**: 3 langkah Outstand Media API digabung jadi **1 method ACL** (bukan split 2-method seperti ADR-105 — tidak ada redirect browser di sini, murni server-to-server). Scope sengaja belum di-wire ke `UploadMediaUseCase`/UI. Lolos review Ridwan (0 temuan; 1 catatan forward-looking non-blocking soal kemungkinan split kontrak nanti kalau Real adapter butuh retry granular). Verifikasi akhir: `typecheck` 0 error, 346 test pass/5 skip. Task T-024 tetap `🟡 In Progress (3/5)`. Detail: `tasks/v02-publishing-mvp.md` § T-024, `decisions/ADR-106-fake-media-upload-working-copy-1-method-gabungan.md`.
---
