...
* **Top Next Tasks:** **T-101 Migrasi Publish — Calendar, Queue, Drafts, Dashboard — 🟡 In Progress (ADR-097)**, T-101.1 (Calendar) dan T-101.2 (Queue) selesai (2026-09-03), lolos verifikasi visual Mark UI Engineer + review Ridwan (0 temuan tiap subtask); re-evaluasi **KI-035** poin 1 Resolved, poin 3 tetap Open — lihat `TASKS.md`/`tasks/v07-astryx-shadcn-migration.md` § T-101 untuk detail. Subtask berikutnya: T-101.3 (Drafts), T-101.4 (header/tabbar/layout), T-101.5 (Dashboard), lalu **T-102**. T-100 Migrasi Publish — Draft Editor Modal sudah ✅ Done (2026-09-03). T-025 Real OutstandAdapter dan T-036 In-app notification + Supabase Realtime (🟡 In Progress, T-036.1–.3 selesai; T-036.4 dibuka kembali untuk verifikasi visual, tersisa juga T-036.5 trigger dari webhook) tetap di antrean — salinan ID dari **Fokus sekarang** di [`TASKS.md`](TASKS.md), yang merupakan satu-satunya daftar fokus
...
* **Prioritas utama saat ini (2026-09-02):** migrasi UI component system
  ...
  Progress` (2/5 subtask); T-102 tersisa.
