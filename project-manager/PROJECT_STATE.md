...
* **T-027 Done (5/5 subtask) — Job runner + Railway Cron, ADR-108, ADR-109 (2026-09-17)** — job runner generik (klaim job `SELECT FOR UPDATE SKIP LOCKED` + registry handler per job type, T-027.1), auth `X-Job-Secret` (T-027.2), retry backoff 5m/15m/60m + dead-letter (T-027.3), Railway Cron config-as-code `railway.json`/`railway.cron.json`/`scripts/trigger-job-run.ts` (T-027.4 — provisioning project Railway sungguhan masih blocked **KI-025**), dan job handler baru `ResolveScheduledPostOutcomeJobHandler` (T-027.5) yang mengenqueue job type baru `publishing.scheduled_post.resolve_outcome` (**JOB-07**, ditambahkan ke `background-jobs.md`) begitu post terjadwal due, lalu reuse `OutstandWebhookProcessor.resolvePostOutcome` (T-026, di-extract jadi public). QA Najwa QA Engineer menemukan bug correctness: post terjadwal tidak pernah resolve outcome karena `FakeOutstandAdapter` (ADR-059) menyimpan state `Map` in-memory yang pecah lintas Server Action↔Route Handler terpisah (dikonfirmasi nyata di production build) — King Rezi memilih root-cause fix (`AskUserQuestion`), dicatat **ADR-108** (`fetchPostOutcome` menambah parameter `expectedOutstandAccountIds`, `FakeOutstandAdapter` jadi pure function tanpa state). Gap pre-existing T-026 juga ditemukan: `PublishingPost.status` tidak pernah bertransisi ke `Published` di level post — dicatat **ADR-109** (method baru `markPostPublished`, melengkapi T-026 tanpa reopen). Review Ridwan Architecture Reviewer 2 putaran (putaran 1: 1 bug diperbaiki; putaran 2: 0 temuan, plus rekomendasi JOB-07 masuk baseline — sudah ditindaklanjuti). QA Najwa end-to-end (browser + `curl` simulasi Railway Cron): golden path Schedule → History Published PASS 2x, regresi Publish Now & Retry manual PASS. **KI-062 baru** (`background-jobs.md` formula backoff self-contradictory) dan **KI-063 baru** (`publishedAt` tidak diisi, minor) dicatat, belum diperbaiki. Verifikasi: `typecheck`/`lint`/`prettier` bersih, `vitest` **396 pass/5 skip**. Detail: `tasks/v02-publishing-mvp.md` § T-027, `decisions/ADR-108-redesain-fetchpostoutcome-expected-account-ids.md`, `decisions/ADR-109-markpostpublished-post-level-status-transition.md`.
---
