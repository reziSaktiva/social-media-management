...
* **T-043 Done (4/4 subtask) — Post performance metrics (2026-09-18)** — Table sortable di `/analyze` (4 kolom Post/Akun/Reach/Eng. Rate, mengikuti pola design-prep sesi terpisah 2026-09-18), metrik di halaman detail post History (reuse `PostMetricsPort`, komponen `MetricTile` diekstrak jadi shared `post-metric-tile.tsx`), dan kriteria "belum ada data" (post tanpa metrik tetap ditampilkan dengan field nullable, bukan di-skip). Rangkaian: Prabowo Feature Engineer → Mark UI Engineer → Ridwan Architecture Reviewer 2 putaran (putaran 1: **kritis** — `PublishingHistoryPort` baru arah analytics→publishing bikin circular dependency dengan `PostMetricsPort` arah publishing→analytics yang sudah ada sejak T-033.1; King Rezi memilih **refactor** via `AskUserQuestion`, bukan ADR menerima circular dependency — `getPostPerformance` dipindah ke `PublishingService` reuse `PostMetricsPort`, dependency jadi satu arah; putaran 2: kedua temuan tertutup, 0 blocking baru) → Najwa QA Engineer (0 temuan blocking, test baru `post-performance-period-range.test.ts` 8 test). **KI-064 baru** (Open): `application-layer.md` § Peta Dependency Antar Domain belum pernah mencantumkan panah `publishing→analytics` sejak T-033.1 — gap dokumentasi lama, baru ketahuan lewat review ini. Catatan non-blocking: metrik tersembunyi History Detail untuk post `Failed` belum terverifikasi visual browser (tidak ada data dev), hanya tertutup unit test. Verifikasi: `typecheck`/`lint` bersih, `vitest` **411 pass/5 skip** (42 file). Detail: `tasks/v03-analytics-mvp.md` § T-043.
---
