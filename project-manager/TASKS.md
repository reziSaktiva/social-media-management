...
| **v0.7** Migrasi Astryx → shadcn/ui | Cross-cutting: ganti fondasi UI component system (ADR-097) | T-095–T-103, T-105¹ | 10   | 🟡 8 ✅ · 2 ⏳ | [tasks/v07-astryx-shadcn-migration.md](tasks/v07-astryx-shadcn-migration.md) |

**Total:** 90 task · 54 selesai · 221 subtask terdefinisi (v0.1–v0.3, v0.7).
...
> post per akun (awalnya port `PublishingHistoryPort` analytics→publishing,
> direfactor jadi method `getPostPerformance` di `PublishingService` reuse
> `PostMetricsPort` yang sudah ada, arah dependency satu jalur
> `publishing→analytics`), UI Table sortable di `/analyze` (4 kolom
> Post/Akun/Reach/Eng. Rate), kriteria "belum ada data" (post tanpa metrik
> tetap disertakan dengan field nullable, bukan di-skip), dan metrik di
> halaman detail post History (reuse `PostMetricsPort`, komponen
> `MetricTile` diekstrak jadi shared `post-metric-tile.tsx`). Rangkaian:
> Prabowo Feature Engineer → Mark UI Engineer → Ridwan Architecture
> Reviewer (2 putaran — putaran 1 menemukan circular dependency
> analytics↔publishing, King Rezi memilih refactor via `AskUserQuestion`;
> putaran 2 kedua temuan tertutup, 0 blocking baru) → Najwa QA Engineer
> (0 temuan blocking, test baru `post-performance-period-range.test.ts`).
> **KI-064 baru** (Open): `application-layer.md` § Peta Dependency Antar
> Domain belum pernah mencantumkan panah `publishing→analytics` sejak
> T-033.1 — gap dokumentasi lama, baru ketahuan lewat review ini. Verifikasi
> akhir: `typecheck`/`lint` bersih, `vitest` **411 pass/5 skip** (42 file).
> Breakdown v0.3 berubah dari "🟡 3 ✅ · 3 ⏳" menjadi **🟡 4 ✅ · 2 ⏳**
> (dihitung ulang langsung dari `tasks/v03-analytics-mvp.md`). Total task
> selesai naik 44 → **45**. **Koreksi hitungan subtask** (dihitung ulang
> langsung dari `tasks/vXX-*.md`, bukan asumsi): total subtask terdefinisi
> ternyata **215**, bukan 217 seperti tercatat sebelumnya — selisih 2 sudah
> ada sebelum sesi ini (bukan disebabkan perubahan T-043), baru ketahuan
> saat audit hitungan sesi ini. Detail: `tasks/v03-analytics-mvp.md` § T-043.
>
...
>
>
> **Update (2026-09-14):** **T-024** (Media upload di Draft Editor) mulai
> dikerjakan — feasibility check menemukan T-024 tidak perlu menunggu T-025
> (Real OutstandAdapter) penuh; subtask yang tidak butuh kredensial Outstand
> asli bisa jalan duluan (rule 19 AGENTS.md, pola ADR-059/ADR-105), hanya
> T-024.3 yang tetap terikat T-025 (akan lewat `FakeOutstandAdapter` dulu
> bila didahulukan). Sebelum implementasi, dicek ke Claude Design (rule 17):
> rancangan Draft Editor sudah ada tapi pola dropzone media belum dikunci ke
> primitive shadcn konkret — King Rezi dikonfirmasi via `AskUserQuestion` 2
> keputusan (dropzone custom, "Pilih dari Media Library" ditunda dari scope
> T-024). **T-024.1** (domain `media` skeleton — service, repository
> interface, implementasi Prisma, `MediaType`/`asMediaId` di
> `packages/shared`, 6 unit test baru) selesai, lolos review arsitektur
> Ridwan (0 temuan) dan verifikasi independen (`typecheck` 0 error, 338 test
> pass/5 skip). Task T-024 pindah dari `⏳ Not Started` ke `🟡 In Progress`
> (1/5 subtask). Breakdown v0.2 berubah dari "16 ✅ · 2 🟡 · 5 ⏳" menjadi
> **16 ✅ · 3 🟡 · 4 ⏳**. Jumlah task/subtask total tidak berubah (87 task,
> 217 subtask) — hanya status yang berubah. Dihitung ulang langsung dari
> `tasks/v02-publishing-mvp.md`, sesuai aturan maintenance.
>
> **Update (2026-09-14):** **T-024.4** (aktifkan kontrol lampiran media di
> Draft Editor + preview) selesai (Prabowo Feature Engineer, full-stack
> Server Action → service → repository → UI). King Rezi mengonfirmasi 2
> keputusan baru lewat `AskUserQuestion`: Draft Editor mendukung
> **multi-media (carousel)**, batas jumlah TERGANTUNG `ContentFormat`
> (`Post` maks 10, `Reel`/`Story`/`Pin` maks 1, batas efektif = MINIMUM
> lintas format kalau draft menargetkan beberapa akun berformat berbeda
> sekaligus) — mengamandemen ADR-039 → **ADR-107 baru**; dan "Pilih dari
> Media Library" tetap ditunda (link disabled tooltip "Coming soon").
> Review Ridwan Architecture Reviewer 2 putaran: putaran 1 menemukan 1
> temuan MEDIUM (`resolveAndValidateMediaIds` dkk meng-collapse "field
> tidak dikirim" jadi `[]`, mengaburkan partial-update semantics
> `mediaIds`), sudah diperbaiki (12 unit test baru); putaran 2 verifikasi
> ulang: 0 temuan. Verifikasi akhir: `typecheck` 0 error, `lint` 0
> error/warning, `vitest` **374 pass/5 skip** (naik dari 362). **KI-059
> baru**: verifikasi manual browser end-to-end (upload → save draft →
> reopen edit → preview restore → validasi batas count per format) belum
> bisa dilakukan sesi ini — dev server gagal start di worktree ini karena
> `DATABASE_URL` tidak tersedia; dicatat sebagai gap yang perlu ditutup
> sebelum T-024 dianggap teruji penuh. Task T-024 tetap `🟡 In Progress`,
> naik dari 3/5 ke **4/5 subtask**. Breakdown task-level v0.2 tidak berubah
> (T-024 sudah dihitung sebagai salah satu dari "3 🟡" sejak subtask
> pertamanya jalan) — hanya status checklist subtask yang berubah. Jumlah
> task/subtask total tidak berubah (87 task, 217 subtask). Detail:
> `tasks/v02-publishing-mvp.md` § T-024,
> `decisions/ADR-107-batas-maksimum-jumlah-media-per-content-format.md`.
>
> **Update (2026-09-14):** **T-024.5** (Delete Media + dialog konfirmasi
> Tier 2, ADR-049) selesai — subtask **terakhir** T-024, menutup task ini
> **5/5**. King Rezi mengonfirmasi lewat `AskUserQuestion`: tombol "hapus
> dari post ini" T-024.4 (unlink-only, tanpa dialog, tidak hapus data)
> diganti total jadi aksi destruktif (dialog konfirmasi → kalau
> dikonfirmasi, hapus file Storage + record `MediaItem` DB permanen).
> Implementasi (Prabowo Feature Engineer): `DeleteMediaUseCase` baru
> (hapus record DB dulu, baru best-effort hapus file Storage), Server
> Action `deleteMediaAction` (`workspaceId` dari session, anti-IDOR), UI
> `Modal.tsx` reuse `ConfirmActionDialog`/`useConfirmAction` (pola sama
> "Hapus Draft"). Review Ridwan Architecture Reviewer: **0 temuan**.
> Verifikasi akhir: `typecheck` 0 error, `lint` 0 error/warning, `vitest`
> **377 pass/5 skip** (naik dari 374). Task T-024 pindah dari `🟡 In
> Progress (4/5)` ke **`✅ Done (5/5)`** — breakdown v0.2 berubah dari
> "16 ✅ · 3 🟡 · 4 ⏳" menjadi **17 ✅ · 2 🟡 · 4 ⏳** (dihitung ulang
> langsung dari `tasks/v02-publishing-mvp.md`, bukan increment manual).
> Total task selesai naik 42 → **43**. Jumlah task/subtask total tidak
> berubah (87 task, 217 subtask) — hanya status yang berubah. **KI-059**
> cakupannya diperluas mencakup T-024.5 (bukan cuma T-024.4) — gap
> verifikasi manual browser end-to-end sama-sama belum bisa ditutup sesi
> ini karena `DATABASE_URL` tidak tersedia. Detail:
> `tasks/v02-publishing-mvp.md` § T-024.
>
> **Update (2026-09-15) — KI-059 Resolved:** verifikasi manual browser
> end-to-end T-024 (yang dicatat KI-059 sebagai belum bisa dilakukan)
> akhirnya dijalankan di worktree `staging-checkout-tasks-976e14` — root
> cause blocker `DATABASE_URL` sebelumnya ternyata dev server sempat start
> dari `cwd` repo `main` yang salah, bukan env var yang benar-benar hilang.
> Seluruh 6 kriteria PASS: upload media (signed URL Supabase Storage asli),
> Save as Draft, reopen edit, preview restore, validasi batas count (10
> media format `Post` per ADR-107, file ke-11 ditolak), dan hapus permanen
> (diverifikasi langsung di level DB via `withCurrentUser`/RLS — record
> benar-benar hilang dari `media_items`). T-024 sekarang teruji penuh
> end-to-end, task/subtask total tidak berubah. **KI-060 baru ditemukan**
> (di luar scope T-024, bukan regresi): Account Selector tidak ter-restore
> saat reopen edit draft. Detail: `COMPLETE_TASK.md` (2026-09-15),
> `tasks/v02-publishing-mvp.md` § T-024.
>
...
| **T-024** | Media upload di Draft Editor                     | ✅ 5/5 | **Done (2026-09-14)** — seluruh 5/5 subtask tuntas. **T-024.1** (domain `media` skeleton) dan **T-024.2** (upload ke Supabase Storage, batas ukuran 50MB) selesai, lolos review Ridwan (0 temuan). Migration bucket `media` belum di-deploy (`bun run db:deploy` pending). **T-024.3** (`OutstandAdapter` media upload working copy) selesai lewat `FakeOutstandAdapter` — 1 method ACL gabungan dicatat sebagai **ADR-106**, lolos review Ridwan (0 temuan). **T-024.4** (aktifkan kontrol lampiran + preview) selesai — Draft Editor mendukung **multi-media (carousel)**, batas jumlah per `ContentFormat` dikonfirmasi King Rezi via `AskUserQuestion` (**ADR-107 baru**, amandemen ADR-039: `Post` maks 10, `Reel`/`Story`/`Pin` maks 1, batas efektif = MINIMUM lintas format kalau multi-target). 2 putaran review Ridwan (1 temuan MEDIUM soal partial-update semantics `mediaIds`, diperbaiki + 12 unit test baru; putaran 2: 0 temuan). **T-024.5** (Delete Media + dialog konfirmasi Tier 2, ADR-049, subtask terakhir) selesai — tombol "hapus dari post ini" T-024.4 (unlink-only) diganti total jadi aksi destruktif (dialog konfirmasi → hapus file Storage + record `MediaItem` DB permanen), reuse `ConfirmActionDialog`/`useConfirmAction` pola "Hapus Draft". Lolos review Ridwan (0 temuan). Verifikasi akhir keseluruhan task: `typecheck`/`lint` bersih, `vitest` **377 pass/5 skip**. Scope dipersempit sejak awal (`AskUserQuestion`): dropzone custom, "Pilih dari Media Library" ditunda (link disabled tooltip "Coming soon"). **KI-059 Resolved (2026-09-15)**: verifikasi manual browser end-to-end T-024 sudah dilakukan, seluruh 6 kriteria PASS — T-024 sekarang teruji penuh end-to-end. **KI-060 baru** (Open, di luar scope T-024): Account Selector tidak ter-restore saat reopen edit draft. **KI-061 baru** (Open): tidak ada warning UI saat media over-limit setelah ganti target akun/format (ADR-107). Lihat `tasks/v02-publishing-mvp.md` § T-024 |
