...
| **T-024** | Media upload di Draft Editor                     | 🟡 2/5 | **In Progress (2026-09-14)** — feasibility check: T-024 tidak lagi menunggu T-025 penuh, subtask yang tidak butuh kredensial Outstand asli dijalankan duluan (rule 19 AGENTS.md, pola ADR-059/ADR-105). **T-024.1** (domain `media` skeleton) dan **T-024.2** (upload ke Supabase Storage, batas ukuran 50MB) selesai, lolos review Ridwan (0 temuan). Migration bucket `media` belum di-deploy (`bun run db:deploy` pending). Scope dipersempit (`AskUserQuestion`): dropzone custom, "Pilih dari Media Library" ditunda. Lihat `tasks/v02-publishing-mvp.md` § T-024 |
