...
| **v0.2** Publishing MVP    | Draft, Format, Schedule, Queue, Calendar, History  | T-020–T-038, T-090¹–T-092¹, T-104¹ | 23   | 16 ✅ · 3 🟡 · 4 ⏳ | [tasks/v02-publishing-mvp.md](tasks/v02-publishing-mvp.md) |
...
| **T-024** | Media upload di Draft Editor                     | 🟡 4/5 | **In Progress (2026-09-14)** — feasibility check: T-024 tidak lagi menunggu T-025 penuh, subtask yang tidak butuh kredensial Outstand asli dijalankan duluan (rule 19 AGENTS.md, pola ADR-059/ADR-105). **T-024.1** (domain `media` skeleton) dan **T-024.2** (upload ke Supabase Storage, batas ukuran 50MB) selesai, lolos review Ridwan (0 temuan). Migration bucket `media` belum di-deploy (`bun run db:deploy` pending). **T-024.3** (`OutstandAdapter` media upload working copy) juga selesai lewat `FakeOutstandAdapter` — 1 method ACL gabungan dicatat sebagai **ADR-106**, lolos review Ridwan (0 temuan), belum di-wire ke `UploadMediaUseCase`/UI. **T-024.4** (aktifkan kontrol lampiran + preview) selesai (Prabowo Feature Engineer) — Draft Editor kini mendukung **multi-media (carousel)**, batas jumlah per `ContentFormat` dikonfirmasi King Rezi via `AskUserQuestion` (**ADR-107 baru**, amandemen ADR-039: `Post` maks 10, `Reel`/`Story`/`Pin` maks 1, batas efektif = MINIMUM lintas format kalau multi-target). 2 putaran review Ridwan (1 temuan MEDIUM soal partial-update semantics `mediaIds`, sudah diperbaiki + 12 unit test baru; putaran 2: 0 temuan). Verifikasi: `vitest` 374 pass/5 skip. **KI-059 baru**: verifikasi manual browser end-to-end belum bisa dilakukan (`DATABASE_URL` tidak tersedia di worktree ini) — gap ini perlu ditutup sebelum T-024 dianggap teruji penuh. Scope dipersempit (`AskUserQuestion`): dropzone custom, "Pilih dari Media Library" ditunda (link disabled tooltip "Coming soon"). Lihat `tasks/v02-publishing-mvp.md` § T-024 |
