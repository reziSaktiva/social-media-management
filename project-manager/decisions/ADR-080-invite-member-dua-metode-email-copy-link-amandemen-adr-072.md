...
1. **Invite member mendukung dua metode**, bukan satu:
   - **Copy Link** — `inviteMember` generate invitation record (`workspace_invitations`, sudah ada dari ADR-072) + link undangan (`/invite/[token]`). Owner/Admin menyalin link ini secara manual dan membagikannya lewat channel apa pun di luar sistem (WhatsApp, Slack, dll). Metode ini **tidak bergantung pada T-005** dan menjadi jalur aktif sekarang.
...
4. **Dependency T-007 → T-005 diturunkan dari hard blocker menjadi soft dependency**: T-007.1 bisa dan harus dikerjakan sekarang (jalur Copy Link), T-005 hanya memblokir aktivasi opsi "Kirim via Email", bukan seluruh fitur invite member.
