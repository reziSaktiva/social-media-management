// Route `/analyze` (T-043.2 + T-046.2 + T-047.1 + T-044, KSP-07 Analyze →
// Dashboard) — Server Component tipis: panggil Server Action
// `getPostPerformanceAction`, `getAccountOverviewAction`,
// `getAnalyzeSummaryAction` (T-047.1 — summary row 3 stat card), DAN
// `getEngagementSummaryAction` (T-044 — card "Engagement Summary", kolom
// kanan `.dash-cols`) dengan period default ("weekly", sama seperti
...
// Keempat action dipanggil paralel (`Promise.all`) — sama-sama scoped
...
export default async function Page() {
  const [postPerformanceRows, accountOverviewRows, summary, engagementSummary] =
    await Promise.all([
