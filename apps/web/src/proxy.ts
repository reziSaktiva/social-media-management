...
// `/api/webhooks` (T-026, ADR-040) ditambahkan ke daftar bypass ini karena
// Outstand mengirim request server-to-server tanpa cookie session sama
// sekali (bukan browser) — otentikasinya lewat HMAC-SHA256 signature yang
// divalidasi di dalam route handler webhook itu sendiri, sama seperti pola
// `/api/v1` di atas. Tanpa bypass ini, gate session di bawah me-redirect
// 307 ke /login SEBELUM request Outstand pernah sampai ke handler webhook —
// ditemukan Najwa QA lewat curl POST asli (v0.2 T-026 Bug 1).
